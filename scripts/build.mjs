...
<section class="section featured-action"><div class="container featured-action-grid"><div class="featured-action-photo">${picture(featuredAction)}<span class="image-source">Imagem publicada pelo canal oficial do ELO.UFRB</span></div><div class="featured-action-copy">${tag("Campanha · " + featuredAction.year)}<p class="place">${esc(featuredAction.location)}</p><h2>${esc(featuredAction.title)}</h2><p>${esc(featuredAction.description)}</p></div></div></section>
...
<section class="section home-actions"><div class="container"><div class="section-heading"><div>${tag("Memória do projeto")}<h2>Ações registradas</h2></div>${link("/acoes/", "Ver todas as ações")}</div><div class="editorial-list"><div class="editorial-list-static"><span class="list-date">${featuredAction.year}</span><span><small>${esc(featuredAction.type)} · ${esc(featuredAction.location)}</small><strong>${esc(featuredAction.title)}</strong></span></div><a href="/acoes/reenconcavo-2023/"><span class="list-date">09<br>MAR<br>2023</span><span><small>Painel · CAHL/UFRB</small><strong>Movimentos Sociais e Universidade: interação em redes</strong></span>${next}</a></div></div></section>
<section class="section home-agenda"><div class="container agenda-strip"><div>${tag("Agenda")}<time datetime="${esc(nextEvent.date)}"><strong>22</strong><span>set<br>2026</span></time></div><div><p class="event-type">${esc(nextEvent.type)} · ${esc(nextEvent.location)}</p><h2>${esc(nextEvent.title)}</h2><p>${esc(nextEvent.time)} · com ${esc(nextEvent.speaker)}</p></div>${link("/agenda/", "Ver detalhes")}</div></section>
<section class="section home-acervo"><div class="container home-acervo-grid"><div>${tag("Publicações")}<h2>Documentos e materiais</h2><p>Produções do projeto reunidas para consulta.</p>${link("/publicacoes/", "Ver publicações")}</div><a class="document-feature" href="${pdf}" download><span>Documento institucional</span><strong>Projeto ELO.UFRB<br>2025 a 2027</strong><small>PDF · 18 páginas</small>${arrow}</a></div></section>
${channels()}`;
